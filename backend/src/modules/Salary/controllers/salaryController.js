...
export const SalaryController = {
  saveSalarySlipPermission: async (req, res) => {
    try {
      ...
      if (
        !user_id ||
        !dept_ids ||
        !Array.isArray(dept_ids) ||
        dept_ids.length === 0
      ) {
        return res.status(400).json({
          status: false,
          message:
            "User ID and Department IDs are required, and dept_ids must be an array.",
        });
      }

      // Call the service to save the salary slip permissions for the multiple departments
      const result = await SalaryService.saveSalarySlipPermission(
        user_id,
        dept_ids
      );
    ...
    try {
      ...
      if (!salary_slip_per_id || !user_id || !dept_id) {
        return res.status(400).json({
          status: false,
          message:
            "Salary Slip Permission ID, User ID, and Department ID are required.",
        });
      }

      const result = await SalaryService.updateSalarySlipPermission(
        salary_slip_per_id,
        user_id,
        dept_id
      );
  ...
  deleteSalarySlipPermission: async (req, res) => {
    try {
      ...
      const result = await SalaryService.deleteSalarySlipPermission(
        salary_slip_per_id
      );
  ...
  checking_salary_slip_per: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ status: true, data: result, message: "Override deleted" });
    } catch (error) {
